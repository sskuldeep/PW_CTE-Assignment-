...
-- ans)  in this database "film " table seems like unnormalize form so we discuss the normalizetion process up to 2nf -- This table -- may not be in 1st Normal Form (1NF) because it has repeating groups (like special_features) -- and multiple values in a single column. To normalize it, we can follow these steps:
...
-- 6) Use a recursive CTE to generate a hierarchical list of categories and their subcategories from the category table in Sakilaa

-- ans) 
WITH RECURSIVE CategoryHierarchy AS ( SELECT category_id, name AS category_name, NULL AS parent_category_id, 0 AS depth FROM category WHERE parent_id IS NULL
...
-- 14) H Implement a recursive CTE to find all employees in the staff table who report to a specific manager, considering the column. 
...
WITH RECURSIVE EmployeeHierarchy AS (
    ...
    FROM 
        staff 
    WHERE 
        manager_id = 'MANAGER_ID'  -- Replace with the actual manager's ID
...
-- Select all employees under the specific manager
